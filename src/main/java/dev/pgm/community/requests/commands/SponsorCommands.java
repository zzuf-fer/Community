...
public class SponsorCommands extends CommunityCommand {
  ...
  public SponsorCommands(FeatureManager features) {
    this.requests = features.getRequests();
    this.users = features.getUsers();
  }

  @CommandMethod(CMD_NAME)
  public void info(CommandAudience audience, Player sender) {
    Component header =
    ...
    requests
        .getRequestProfile(sender.getUniqueId())
        .thenAcceptAsync(
            profile -> {
                  ...
                  text()
                      ...
                      .append(
                          text(
                              ...
                              NamedTextColor.GOLD))
                      .append(renderRefreshTime(sender, profile))
              ...
              requests
                  .getPendingSponsor(sender.getUniqueId())
                  .ifPresent(
                      sponsor -> {
              ...
              // Cooldown message
              if (!requests.canSponsor(sender.getUniqueId())) {
                audience.sendMessage(
                    text()
